...
$$
\max \sum_{t=1}^{T} r_t, \quad r_t \sim R(\cdot \mid a_t)
$$
...
$$
a_t = \begin{cases} \arg\max_{a \in A} \hat{Q}(a), & p = 1-\epsilon \\\\ \operatorname{random}_{a \in A}, & p = \epsilon \end{cases}
$$
...
$$
P(E[X] \geq \overline{x}_n + u) \leq e^{-2nu^2}
$$

代入

$$
\hat{Q}_n = \overline{x}_n
$$
...
$$
P(E[X] < \overline{x}_n + \hat{U}(a_n)) = 1-p
$$
...
$$
\hat{U}(a) = \sqrt{\frac{-\log p}{2N(a_t)}}
$$

此时，

$$
a_t = \arg\max_{a \in A}\left[\hat{Q}(a) + c\hat{U}(a)\right]
$$
